...
export default function Results() {
  ...
  const { data: attempts, isLoading } = useQuery({
    ...
    queryFn: () =>
      api
        .get('/exam-attempts?populate=exam&sort=createdAt:desc')
        .then(r => r.data.data),
  })
  ...
  // Vai drīkst rādīt review?
  const canShowSelected =
    selectedAttempt &&
    (
      selectedAttempt.showResults === true ||           // QuickQuiz / justSubmitted shape
      selectedAttempt.exam?.showResults === true        // exam attempts no backend
    )
    ...
    return (
      <div>
        ...
        <h1 className="text-3xl font-bold text-blue-700 mb-2">
          {selectedAttempt.exam?.title || selectedAttempt.title || 'Exam'}
        </h1>

        {typeof selectedAttempt.score === 'number' && (
          <p className="mb-6" style={{ color: 'var(--text-secondary)' }}>
            {selectedAttempt.score}% •{' '}
            {selectedAttempt.passed ? t('results.passed') : t('results.failed')}
          </p>
        )}
  ...
  return (
    <div>
      ...
      {/* Empty state */}
      {inProgressAttempts.length === 0 && submittedAttempts.length === 0 && !justSubmitted && (
        <div
      ...
      {/* Just submitted (Exam vai QuickQuiz) */}
      {justSubmitted && (
        <div
          className="rounded-xl shadow p-5 mb-6"
          style={{ backgroundColor: 'var(--bg-card)' }}
        >
          <h2
          ...
          {justSubmitted.showResults ? (
            <div>
              {typeof justSubmitted.score === 'number' && (
              ...
              >
                {t('results.viewAnswers') || 'View answers'}
              </button>
            </div>
          ) : (
            <p style={{ color: 'var(--text-secondary)' }}>
      ...
      {/* Submitted exam attempts history (no QuickQuiz, jo tā nav DB) */}
      <div className="space-y-4">
        {submittedAttempts.map(attempt => {
          const released =
            attempt.submittedAt && attempt.exam?.showResults === true
          return (
            <div
              key={attempt.id}
              onClick={() => {
